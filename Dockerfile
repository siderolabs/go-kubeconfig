...
#
# Generated on 2025-03-12T10:31:21Z by kres ec5ec04.
...
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-kubeconfig/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-kubeconfig/go/pkg go install github.com/golangci/golangci-lint/cmd/golangci-lint@${GOLANGCILINT_VERSION} \
...
WORKDIR /src
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-kubeconfig/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-kubeconfig/go/pkg govulncheck ./...

# runs unit-tests with race detector
FROM base AS unit-tests-race
WORKDIR /src
ARG TESTPKGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-kubeconfig/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-kubeconfig/go/pkg --mount=type=cache,target=/tmp,id=go-kubeconfig/tmp CGO_ENABLED=1 go test -v -race -count 1 ${TESTPKGS}

# runs unit-tests
FROM base AS unit-tests-run
WORKDIR /src
ARG TESTPKGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-kubeconfig/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-kubeconfig/go/pkg --mount=type=cache,target=/tmp,id=go-kubeconfig/tmp go test -v -covermode=atomic -coverprofile=coverage.txt -coverpkg=${TESTPKGS} -count 1 ${TESTPKGS}
